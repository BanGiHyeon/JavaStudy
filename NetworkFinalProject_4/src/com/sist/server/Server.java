...
public class Server implements Runnable{
	...
	public Server()
	{
		try
		{
			ss=new ServerSocket(PORT);// IP자동인식
			// bind() => listen()
			// 50명까지 한정 
	...
	class Client extends Thread
	{
		...
		Socket s;
		// 클라이언트 요청 / 응답
		BufferedReader in; // 용청받은 경우
		OutputStream out; // 응답하는 과정
				...
				while(true)
				{
					...
					case Function.LOGIN:
					{
						...
						messageAll(Function.LOGIN+"|"
						           +id+"|"+name+"|"
								   +sex+"|"+pos);
						messageAll(Function.WAITCHAT+"|[알림 ☞] "+name+"님 입장하였습니다");
						// waitVc => 저장
						waitVc.add(this);
		...
		// 처리 방식 => 2개
		// 1. 접속자 전체로 전송 => 로그인(먼저 로그인 사람) / 나가기(남아있는 사람) / 채팅 
