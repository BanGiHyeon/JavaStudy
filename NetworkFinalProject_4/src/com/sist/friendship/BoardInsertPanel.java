...
public class BoardInsertPanel extends JPanel implements ActionListener{
	...
	public BoardInsertPanel(ControllPanel cp)
	{
		...
		JPanel p=new JPanel();
		p.add(b1);p.add(b2);
		p.setBounds(10, 555, 635, 35); // 555 => 위 2번+4번+10
		add(p);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==b2)
		{
			cp.card.show(cp, "board"); // cp.card => b2 버튼을 누르면 목록으로 전환해준다
		}
		else if(e.getSource()==b1)
		{
			...
			vo.setPwd(pwd);
			vo.setHit(0);
			vo.setRegate(new Date());
			bm.boardInsert(vo);
			
			//화면 이동
			cp.card.show(cp, "board");
			cp.blp.boardList(); // 글쓰기 입력 후 목록에 바로 글쓰기 게시물 화면 출력!!
		}
	}
}