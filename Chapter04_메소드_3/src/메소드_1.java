 ...
 *        ---------- 맨 마지막에 사용하는 것이 아니라 중간에서 사용이 가능하다
 *        메소드()
 *        {
 *           if()
 *           {
 ...
 *                             => 중복 정의 메소드
 *        plus(int,int) , plus(char,char) , plus(double,double) => 다른 메소드로 인식
 *    4. 메소드 호출
 *       --------
 *        = 인스턴스(따로 저장) 메소드 => new를 이용해서 메모리 저장
...
public class 메소드_1 {
	...
	private static char[] reverse(int i) {
